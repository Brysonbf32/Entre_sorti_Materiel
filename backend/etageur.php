...
if(isset($_POST['ajouter'])){
	...
	if(isset($nom_eta)){
		$logs= $my_bd->prepare("SELECT * FROM tbl_etagiaire WHERE nom_etage=?");
		$logs->execute(array($nom_eta));
		$logs->rowCount();
		$checks=$logs->fetch();
		if($checks >0){
 			?>
