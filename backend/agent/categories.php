...
if(isset($_POST['ajouter'])){
	...
	if(isset($id_etag) and !empty($code_categ) and !empty($nom_categ)){
		$logs= $my_bd->prepare("SELECT * FROM tbl_categorie WHERE id_etage=? AND ref_categ=?");
		$logs->execute(array($id_etag,$code_categ));
		$logs->rowCount();
		$checks=$logs->fetch();
		if($checks >0){
 			?>
