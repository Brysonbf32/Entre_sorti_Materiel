...
if(isset($_POST['ajouter'])){
	...
	if(isset($nom_pro) and !empty($categ_pro) and !empty($prix_pro)){
		$logs= $my_bd->prepare("SELECT * FROM tbl_produits WHERE id_categ=? AND nom_prod=?");
 			...
 			<?php
				...
				if(in_array($file_extension,$extension_allowed)){
					if(move_uploaded_file($file_tmp_name,$path)){
						...
						$quantipro=0;
						$insert=$my_bd->prepare("INSERT INTO tbl_produits(ref_prod,nom_prod,photo_pro,id_categ,prix_pro)VALUES(?,?,?,?,?)");
						$insert->execute(array($ref_prod,$nom_pro,$file_name,$categ_pro,$prix_pro));
					}
				}
